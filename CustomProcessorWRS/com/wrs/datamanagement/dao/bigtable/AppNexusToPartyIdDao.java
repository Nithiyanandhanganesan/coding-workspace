...
public class AppNexusToPartyIdDao {
    private static final Logger logger = LoggerFactory.getLogger(AppNexusToPartyIdDao.class);
    public static final byte[] bAppnexusUserId = Bytes.toBytes("appnexus_user_id");
    ...
    public static final String tableName = "appnexus_to_partyid";
    ...
    public AppNexusToPartyIdDao(BigTableConnector bigTableConnector) {
        this.bigTableConnector = bigTableConnector;
    }

    public String save(String appNexusUserId, String partyOwner) {
        String partyId = null;
        Table table = null;
        try {
            table = bigTableConnector.getBigTableConnection().getTable(TableName.valueOf(tableName));
            String rowKey = appNexusUserId + "#" + partyOwner;
            partyId = UUID.nameUUIDFromBytes(appNexusUserId.getBytes()).toString();
            String creationDate = DateUtil.formatDateWithString(null);
            Put put = new Put(Bytes.toBytes(rowKey));
            put.addColumn(bytesColumnFamily1, bAppnexusUserId, Bytes.toBytes(appNexusUserId));
            ...
            //put.addColumn(bytesColumnFamily2, bCid, Bytes.toBytes("test1"));
            table.put(put);
            return partyId;
        } catch (IOException e) {
            logger.error("Exception occured while inserting partyid for appnexus {}, {}", appNexusUserId, partyOwner, e);
        }
        finally
        {
            if(table != null)
    ...
    public List<String> saveBatch(List<IdData> objs)
    {
        ...
        try {
            ...
            for(IdData obj: objs) {
                ...
                String creationDate = DateUtil.formatDateWithString(null);
                Put put = new Put(Bytes.toBytes(rowKey));
                put.addColumn(bytesColumnFamily1, bAppnexusUserId, Bytes.toBytes(obj.getId()));
                put.addColumn(bytesColumnFamily1, bPartyId, Bytes.toBytes(partyId));
    ...
    public void delete(String appNexusUserId, String partyOwner) {
    ...
    public String findPartyIdByRowKey(String appnexusUserId, String partyOwner){

        FilterList list = new FilterList(FilterList.Operator.MUST_PASS_ALL);
        Filter rowFilter = new PrefixFilter(Bytes.toBytes(appnexusUserId));
